...
func (i *Image) TextWordWrap(label string, x, y float64, fontPath string, fontColor color.Color, fontSize float64, align string) *Image {
	...
	dc := NewContext(i.width, i.height)
	...
	switch align {
	case "AlignLeft":
		dc.DrawStringWrapped(label, x, y, 0.0, 0.0, float64(i.width), 1.5, AlignLeft)
	case "AlignRight":
		dc.DrawStringWrapped(label, x, y, 0.1, 0.0, float64(i.width), 1.5, AlignRight)
	case "AlignCenter":
		dc.DrawStringWrapped(label, x, y, 0.5, 0.5, float64(i.width), 1.5, AlignCenter)
	default:
		dc.DrawStringWrapped(label, x, y, 0.0, 0.0, float64(i.width), 1.5, AlignLeft)
	}
	img := dc.Image()
